function ll_fit_map(mag_file_name, t1_file_name, mask_file_name, seqParams)
%LL_FIT_MAP Fits Look-Locker maps using magnitude data only by flipping
...
TR = cell2mat(h_hdr.details.acquisition.attvalue(getAttributeIndexNiak(h_hdr,'repetition_time')));
alpha= 180; % Assume Perfect 180
beta = cell2mat(h_hdr.details.acquisition.attvalue(getAttributeIndexNiak(h_hdr,'flip_angle')));
Nll = num_echoes;
...
h_hdr.details = temp;
h_hdr.info.dimensions = h_hdr.info.dimensions(1:3);
h_hdr.info.dimension_order = h_hdr.info.dimension_order(1:3);
